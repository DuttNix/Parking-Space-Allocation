...
def detect(save_img=False):
    ...
    with open(opt.parking_box_label) as file:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        label = f'{names[int(cls)]}' #label = f'{names[int(cls)]} {conf:.2f}'
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=1)
                    #saving it in an array to compare the boxes
                    frameBox.append(*torch.tensor(xyxy).view(1, 4).tolist()) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='aiina_case_1/aiina_case_1.mp4', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--parking_box_label', type=str, default='aiina_case_1/aiina_case_1.yaml', help='parking_box_label file')    
